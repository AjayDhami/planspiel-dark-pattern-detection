...
const PatternCard: React.FC<PatternCardProps> = ({patternKey , websiteId, patternId, isAutoGenerated, loggedInExpert, pattern_date, pattern_type, expertName, color, text, hoverText}) => {
    ...
    const openPatternModal = () => {
        if(token) {
            const patternObj = getSpecificPattern(patternId , websiteId, token);
  ...
  return (
    <div className={`md:col-span-1 shadow-xl py-1 rounded-xl bg-white mt-4 `} key={patternKey}>
        ...
        <div className='flex justify-between py-1 px-3 my-2'>
          <button className={`px-8 py-1 bg-gradient-to-r from-cyan-500 to-blue-500 rounded-lg shadow-lg text-white`} onClick={openPatternModal}>View Details</button>
